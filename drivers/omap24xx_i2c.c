...
// Functions generic_i2c_read() and generic_i2c_write() are added to support I2C devices
...
static void wait_for_bb(void)
{
	...
	if (timeout <= 0) {
		...
		*/
		w = inw(I2C_SYSTEST);
		outw(I2C_CON_EN, I2C_CON);
		outw((I2C_SYSTEST_ST_EN | I2C_SYSTEST_TMODE_TEST), I2C_SYSTEST);
		...
		if(timeout <= 0)
			printf("I2C Bus busy recovery failed!\n");
		else
			printf("I2C Bus recovered from bus busy.\n");
	}
...
static u16 wait_for_pin(void)
{
	...
	do {
		// Changed the delay from 1000 (1ms) to 100 (0.1ms), and timeout from 10 to 100.
		// NXP's PN544 (part of PN65) requires inter-byte delay be less than 0.45ms
		udelay(100);
		status = inw(I2C_STAT);
	} while (!(status &
